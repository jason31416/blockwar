"""
server v 1.0.1, blockwar v 1.4.3
"""
version = (1, 4, 3)
...
class bullet(obj):
    ...
    def update(self, gm: pyge.Game):
        ...
        if world[int(self.x)][int(self.y)] == 5:
            ...
            gm.rem_obj(self.name)
            blockchange(int(self.x), int(self.y))
            return
...
def addblockchange(x, y):
    events.append(("bc", x, y, world[x][y], wattr[x][y][0], wattr[x][y][1], wattr[x][y][2]))

def blockchange(x, y):
    if (x, y) not in needblockchange:
        needblockchange.append((x, y))

def entitychange(ent: player):
    if ent not in needentchange:
        needentchange.append(ent)

def addentitychange(ent: player):
    events.append(("ec", ent.name, ent.x, ent.y, ent.hp))

def entityremove(ent: player):
    events.append(("er", ent.name))

def entitycreate(ent: player):
    events.append(("ecr", ent.name, ent.team, ent.hp, ent.x, ent.y))
...
def refresh_wd(tk):
    ntid = tk%(wsize*wsize)
    addblockchange(ntid//wsize, ntid%wsize)
    # print(ntid//wsize, ntid%wsize)
...
class game(pyge.Game):
    ...
    def update_back(self):
        global x, y, vx, vy, blksz, nowevst, nwevents, events, begintime, tks
        tks = self.tick
        ...
        if self.now_page == "main":
            begintime = time.time()
            if clientnum != 0:
                for i in needblockchange:
                    addblockchange(i[0], i[1])
                for i in needentchange:
                    addentitychange(i)
                rfpt = 5
                for i in range(rfpt):
                    refresh_wd(self.tick * rfpt+i)

            needblockchange.clear()
            needentchange.clear()
            if len(nowevst) >= clientnum:
                nwevents = events
                events = []
                nowevst = []
            will_draw = []
...
class server(socketserver.BaseRequestHandler):
    ...
    def get(self):
        dt = self.request.recv(1024)
        return dt.decode("utf-8")

    def send_packet(self):
        while self.id in nowevst:
            time.sleep(0.03)
        dt = []
        for i in nwevents:
            dt.append(" ".join([str(j) for j in i]))
        dt.append("!end")
        # print(len(dt))
        for i in dt:
            self.send(i)
            if self.get() != "ok":
                print("Failed to send line "+i)
                return False
        nowevst.append(self.id)
        return True

    def handle_packet(self):
        kevs = self.get()
        plobj = gm.get_obj(self.nm)
        for kev in kevs.split(" "):
            if kev == "w":
                plobj.moved(0, -1)
            elif kev == "s":
                plobj.moved(0, 1)
                ...
                plobj.shot(gm, float(kev.split(",")[1]), float(kev.split(",")[2]))
            if kev != "n" and kev != "":
                entitychange(plobj)
        if self.lstchecktick != gm.tick:
            self.lstchecktick = gm.tick
            plobj.check(gm)

    def initialize(self):
        print("initializing connection "+self.client_address[0])
        rq: socket.socket = self.request
        dt = f"{wsize} {teamn}\n-\n"
        for i in world:
            for j in i:
                dt += str(j) + " "
        ...
        for i in entity.keys():
            for j in entity[i]:
                j: player
                dt += f"{j.x},{j.y},{j.hp},{j.team},{j.name}\n"
        dt += "-\n"
        for i in all_ckp:
            dt += f"{i[0]},{i[1]},{ckp_nm[all_ckp.index(i)]}\n"
        dt += "!end"
        for i in dt.split("\n"):
            self.send(i)
            if self.get() != "ok":
    ...
    def handle(self):
        global clientnum
        print("new connecting request")
        rq: socket.socket = self.request
        self.send("hello")
        if self.get() != f"blockwar v{version[0]}.{version[1]}.{version[2]}":
            rq.close()
            return
        ...
        if not self.initialize():
            ...
            return
        clientnum += 1
        newpl = player(random.randint(0, teamn-1), typ="human")
        # newpl = aiplayer(random.randint(0, teamn))
        gm.add_player(newpl)
        entitycreate(newpl)
        self.send(newpl.name)
        if self.get() != "ok":
            rq.close()
            return
        self.nm = newpl.name
        log(tks, self.client_address[0]+" joined", (255, 255, 0))
        while True:
    ...
    def finish(self) -> None:
        global clientnum
        clientnum -= 1
        log(tks, self.client_address[0]+" disconnected", (255, 255, 0))
        entityremove(gm.get_obj(self.nm))
        gm.rem_obj(self.nm)

sv = socketserver.ThreadingTCPServer(("localhost", 25465), server)
threading.Thread(target=sv.serve_forever).start()
